...
import com.rummytitans.playcashrummyonline.cardgame.models.*
import com.rummytitans.playcashrummyonline.cardgame.ui.appupdate.BottomSheetEvents
...
@BindingAdapter(value = ["setPositionLeftText"], requireAll = true)
fun setPositionLeftText(textView: TextView, leagueData: LeagueData?) {
    val data = leagueData?.NoofMembers?.minus(leagueData?.LeaugeCount ?: 0)
    val text = "$data ${textView.context.getString(R.string.txt_spot_left)}"
    textView.text = text
}
...
fun setCheckBoxTint(view: CheckBox, isCheckBoxSelected: Boolean) {
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
        ...
        else ContextCompat.getColor(view.context, R.color.azure_two)
...
fun addAdapter(spinner: Spinner, dataList: MutableList<String>) {
    if (spinner.adapter == null) {
        val adapter = ArrayAdapter<String>(spinner.context, R.layout.item_states)
...
// TODO : avatar adapter for complete profile
/*
...
fun TextView.setSpannableTexts(
    ...
    try {
        ...
        ss.setSpan(
            ForegroundColorSpan(context.getColorInt(R.color.mainColor)),
            spannableText.indexOf(clickableText),
            spannableText.indexOf(clickableText) + clickableText.length,
            Spanned.SPAN_EXCLUSIVE_EXCLUSIVE
        )
...
@BindingAdapter(value = ["setVerificationText"], requireAll = true)
fun setVerificationText(view: TextView, model: ProfileVerificationModel?) {
    if(model?.profileVerified() == true){
        view.text = view.context.getString(R.string.updateDetails)
    }else{
        view.text = view.context.getString(R.string.verify)
    }
}
...
@BindingAdapter(value = ["setVerificationAllDone"], requireAll = true)
fun setVerificationAllDone(view: ImageView, model: ProfileVerificationModel?) {
    if (model?.EmailVerify == true && model.PanVerify && model.BankVerify) view.alpha = 1f
    else view.alpha = 0.1f
}

@BindingAdapter("finalVerificationStatus")
fun isVerificationAllDone(view: TextView, model: ProfileVerificationModel?) {
    if (model?.BankVerify == true && model.PanVerify && model.BankVerify)
...
@BindingAdapter(value = ["onChangeBottomSheetState", "bottomSheetEvent"], requireAll = true)
fun onChangeBottomSheetState(
    view: ConstraintLayout, data: MutableLiveData<Int>, bottomSheetEvent: BottomSheetEvents?
) {
