...
fun SharedPreferenceStorage.toUserDetail(gson: Gson): LoginResponse? {
    ...
    return gson.fromJson(loginResponse, com.rummytitans.playcashrummyonline.cardgame.models.LoginResponse::class.java)
}
...
fun Context.redirectToAppUpdateScreen(versionResp: VersionModel, comeFrom:String){
    val newIntent=if(versionResp.UpdateType== MyConstants.APP_UPDATE_FULL_SCREEN)
        Intent(this, AppUpdateActivity::class.java)
    else
        Intent(this, AppUpdateBottomSheetActivity::class.java)
    startActivity(newIntent.putExtra(MyConstants.INTENT_COME_FROM, comeFrom))
}
