...
import com.rummytitans.playcashrummyonline.cardgame.AdvertisingIdClient
import com.rummytitans.playcashrummyonline.cardgame.R
import com.rummytitans.playcashrummyonline.cardgame.analytics.AnalyticsKey
import com.rummytitans.playcashrummyonline.cardgame.databinding.ActivityNewLoginBinding
import com.rummytitans.playcashrummyonline.cardgame.databinding.DialogRefercodeBinding
...
import com.rummytitans.playcashrummyonline.cardgame.ui.mobileverification.MobileVerificationActivity
...
class NewLoginActivity : BaseActivity(), NewLoginNavigator, OtpReceiver.OTPReceiveListener {
    lateinit var binding: ActivityNewLoginBinding
    lateinit var mViewModel: NewLoginViewModel

    private val REQUEST_CODE_LOGIN = 111
    private  val mOtpReceiver by lazy { OtpReceiver() }
    ...
    override fun showReferCodeDialog() {
      ...
      //TODO : for refercode dialog
        val dialogView: DialogRefercodeBinding? = DataBindingUtil.inflate(
            layoutInflater, R.layout.dialog_refercode, null, false
        )
        dialogView?.run {
            mViewModel.validForRefercode.set(false)//reset
            referCodeDialog = MyDialog(this@NewLoginActivity).getMyDialog(dialogView.root)
            referCodeDialog?.show()
            viewModel = mViewModel
            editText.addTextChangedListener {
                if (it.toString().isNotEmpty())
                    inputLayout1.error = null
                mViewModel.validForRefercode.set(it.toString().isNotEmpty())
            }

            dialogView.txtNo.setOnClickListener {
                hideKeyboardView()
                mViewModel.referCode.set(false)
                referCodeDialog?.dismiss()
                binding.includeMobileNumber.editMobileNumber.clearFocus()
                Handler(Looper.getMainLooper()).postDelayed({ hideKeyboard() }, 100)
            }
            dialogView.txtYes.setOnClickListener {
                hideKeyboardView()
                dialogView.editText.text.toString().let {
                    if (!isValidReferCode(it)) {
                        inputLayout1.error = getString(R.string.invalid_refer_code)
                    } else {
                        inputLayout1.error = null
                        mViewModel.referCode.set(true)
                        mViewModel.userReferCode.set(it)
                        binding.includeMobileNumber.editReferCode.setText(it)
                        referCodeDialog?.dismiss()
    ...
    override fun forgotPassword() {
        mViewModel.analyticsHelper.fireEvent(
            AnalyticsKey.Names.ButtonClick, bundleOf(
                AnalyticsKey.Keys.ButtonName to AnalyticsKey.Values.ForgetPassword,
                AnalyticsKey.Keys.Screen to AnalyticsKey.Screens.LOGIN,
            )
        )
        ...
        if (intent.hasExtra(MyConstants.INTENT_PASS_COMING_FROM)) {
            startActivity(
                Intent(this, MobileVerificationActivity::class.java)
                    .putExtra(MyConstants.INTENT_PASS_COMING_FROM, "contest")
                    .putExtra(MyConstants.INTENT_FORGOT_PASSWORD, true)
                    .putExtra(MyConstants.VARIFY_fROM, MyConstants.FORGET_PASSWORD)
            )
        } else {
            startActivity(
                Intent(this, MobileVerificationActivity::class.java)
    ...
    fun goToHelp(v: View) {
        ...
        //TODO : for handel support from login
        startActivity(
    ...
    private fun startSMSListener() {
        try {
            ...
            registerReceiver(mOtpReceiver, intentFilter)
    ...
    override fun onOTPReceived(otp: String?) {
        binding.includeOTPVerify.otpView.setText(otp.toString())
    }
